...
class JsonWebTokenSpec extends FunSpec with Matchers {
  ...
  describe("JsonWebToken") {
    ...
    it("should produce the same results for all three claims types") {
      ...
      val res1 = JsonWebToken.apply(header, JwtClaimsSet.apply(Map("Hey" -> "foo")), "secretkey")
      val res2 = JsonWebToken.apply(header, JwtClaimsSet.apply("Hey" -> "foo"), "secretkey")
      val res3 = JsonWebToken.apply(header, JwtClaimsSet.apply("{\"Hey\":\"foo\"}"), "secretkey")
