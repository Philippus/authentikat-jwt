...
class JsonWebSignatureSpec extends FunSpec with Matchers {
  describe("JsonWebSignature") {
    ...
    it("should throw exception for missing key") {
      intercept[IllegalArgumentException] {
        JsonWebSignature(HS256, "someData")
      }
    }
    ...
    it("should report an error for an unknown algorithm") {
      case object Abracadabra extends Algorithm
      intercept[UnsupportedOperationException] {
        JsonWebSignature(Abracadabra, "someData", "secretKey")
      }
      intercept[UnsupportedOperationException] {
        JsonWebSignature("Abrcadabra", "someData", "secretKey")
      }
    }
  }
}
